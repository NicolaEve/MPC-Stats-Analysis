...
# symmetry 
boxplot(recent10fff$SymmCrossAT, plot=FALSE)$out
outliers <- boxplot(recent10fff$SymmCrossAT, plot=FALSE)$out
symm_out_rm<-recent10fff
symm_out_rm<- symm_out_rm[-which(recent10fff$SymmCrossAT %in% outliers),]

boxplot(recent10fff$SymmCrossAT, symm_out_rm$SymmCrossAT)
boxplot(recent10fff$SymmInAT, symm_out_rm$SymmInAT)

# flatness crossline
boxplot(recent10fff$FlatCrossAT, plot=FALSE)$out
outliers <- boxplot(recent10fff$FlatCrossAT, plot=FALSE)$out
flat_cross_rm<-recent10fff
flat_cross_rm<- flat_cross_rm[-which(recent10fff$FlatCrossAT %in% outliers),]

# flatness inline
boxplot(recent10fff$FlatInsAT, plot=FALSE)$out
outliers <- boxplot(recent10fff$FlatInAT, plot=FALSE)$out
flat_in_rm<-recent10fff
flat_in_rm<- flat_in_rm[-which(recent10fff$FlatInAT %in% outliers),]

boxplot(recent10fff$FlatInAT, flat_in_rm$FlatInAT)
boxplot(recent10fff$FlatCrossAT, flat_cross_rm$FlatCrossAT)
...
rand_10fff_rm_symm <- symm_out_rm[sample(nrow(symm_out_rm), size=50),]
rand_10fff_rm_flat_in <- flat_in_rm[sample(nrow(flat_in_rm), size=50),]
rand_10fff_rm_flat_cross<- flat_cross_rm[sample(nrow(flat_cross_rm), size=50),]
```
...
boxplot(recent10fff$FlatInAT, recent10fff$FlatCrossAT, main = "Flatness", names = c("10xFFF", "10xFFF"), col = c(2,3))
legend(x = "right", legend = c("Inline", "Crossline"), fill = 2:3, col = c("orange", "red"))
...
boxplot(flat_in_rm$FlatInBT, flat_in_rm$FlatInAT, main = "Flatness Inline (%)", names = c("10fff", "10fff"), col = c(2,3))
legend(x = "right", legend = c("Before Calibration", "After Calibration"), fill = 2:3, col = c("orange", "red"))
...
boxplot(flat_cross_rm$FlatCrossBT, flat_cross_rm$FlatCrossAT, main = "Flatness Crossline (%)", names = c("10fff", "10fff"), col = c(2,3))
legend(x = "right", legend = c("Before Calibration", "After Calibration"), fill = 2:3, col = c("orange", "red"))
...
# 10fff with outliers removed
z.paired(flat_in_rm$FlatInBT, flat_in_rm$FlatInAT)
z.paired(flat_cross_rm$FlatCrossBT, flat_cross_rm$FlatCrossAT)
```
...
```{r}
sum(df10fff$MPC_Distance > 0.5) + sum(df10fff$MPC_Distance < - 0.5)
```
...
# how does it compare to the whole df
# there's so few failures that it might not be representative to take only the msot recent df
sum(recent10fff$uniformity_test == "fail")
(sum(df10fff$Uniformity > 2) + sum(df10fff$Uniformity < -2))/sum(df10fff$Uniformity)
...
plot_mpc_time <- function(df, energy){
  ...
  geom_point() +
    geom_smooth(aes(x = QA_Date, y = Uniformity), method = "loess") +
  xlab("QA Date") +
  ylab("Uniformity") +
  ...
  geom_point() +
    geom_smooth(aes(x = QA_Date, y = MPC_Distance), method = "loess") +
  xlab("QA Date") +
...
# h is the number of points to predict ahead
...
qic(recent6x$Output_Change, x = recent6x$QA_Date, chart = 'xbar')
qic(SymmInAT, x = QA_Date, data = recent6x, chart = 'xbar')
qic(recent6x$SymmInAT, x = recent6x$QA_Date, chart = 'mr')

```

Set up the data as grouped by week
```{r}
# grouped by week
# numbered from start of the year
recent6x$week <- week(recent6x$QA_Date) 
recent10x$week <- week(recent10x$QA_Date) 
recent10fff$week <- week(recent10fff$QA_Date) 
```

Function to plot the spc charts
```{r}

plot_spc <- function(df){

SymmIn <- qcc.groups(df$SymmInAT, df$week)
qcc(SymmIn, type = "R")
qcc(SymmIn, type = "xbar")

SymmCross <- qcc.groups(df$SymmCrossAT, df$week)
qcc(SymmCross, type = "R")
qcc(SymmCross, type = "xbar")

FlatIn <- qcc.groups(df$FlatInAT, df$week)
qcc(FlatIn, type = "R")
qcc(FlatIn, type = "xbar")

FlatCross <- qcc.groups(df$FlatCrossAT, df$week)
qcc(FlatCross, type = "R")
qcc(FlatCross, type = "xbar")

Uniformity <- qcc.groups(df$Uniformity, df$week)
qcc(Uniformity, type = "R")
qcc(Uniformity, type = "xbar")

OutputChange <- qcc.groups(df$Output_Change, df$week)
qcc(OutputChange, type = "R")
qcc(OutputChange, type = "xbar")
qcc(OutputChange, type = "S")
