...
```{r}
pred_uniformity_6x <- auto.arima(training_df6x[,8])
# Fit and forecast with auto.arima()

# frequency is the number of observations per cycle
# number specifies the data column

# h is the number of points to predict ahead
fvals <- forecast(pred_uniformity_6x, h=nrow(testing_df6x))
print(fvals)
plot(fvals)


# root mean squared error comparing the actual to the predicted
# favsl$mean = mean of the prediction intervals
error <- rmse(testing_df6x$Uniformity, fvals$mean)
abs_error <- mae(testing_df6x$Uniformity, fvals$mean)
print(error)
print(abs_error)


 
uniformdf <- data.frame(
  actual = testing_df6x$Uniformity,
  predicted = fvals$mean,
  CL = limits(training_df6x$Uniformity)[1] ,
  LCL <- limits(training_df6x$Uniformity)[2],
  UCL <- limits(training_df6x$Uniformity)[3])

# add a state pass/warning/fail to the df
f <- `$<-`

# uniformity
uniformdf <- f(uniformdf, "pred_state", "pass")
uniformdf$pred_state[uniformdf$predicted <= -2] <- "fail"
uniformdf$pred_state[uniformdf$predicted>2] <- "fail"
uniformdf$pred_state[uniformdf$predicted<uniformdf$UCL....limits.training_df6x.Uniformity..3. & uniformdf$predicted>uniformdf$LCL....limits.training_df6x.Uniformity..2.] <- "warning"

uniformdf <- f(uniformdf, "actual_state", "pass")
uniformdf$actual_state[uniformdf$actual <= -2] <- "fail"
uniformdf$actual_state[uniformdf$actual>2] <- "fail"
uniformdf$actual_state[uniformdf$actual<uniformdf$UCL....limits.training_df6x.Uniformity..3. & uniformdf$actual>uniformdf$LCL....limits.training_df6x.Uniformity..2.] <- "warning"
...
av_accuracy(uniformdf, "warning")
av_accuracy(uniformdf, "pass")
av_accuracy(uniformdf, "fail")

```
```{r}
plot(uniformdf$predicted) +
  plot(uniformdf$actual) + 
  plot(uniformdf$LCL....limits.training_df6x.Uniformity..2.)
