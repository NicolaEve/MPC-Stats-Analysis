...
recent10x$centre_cal_test[recent10x$Calibrated_Distance<centre_threshold & recent10x$Calibrated_Distance>-centre_threshold] <- "pass"
  ...
  # uniformity
recent10x$uniformity_test <- "fail"
recent10x$uniformity_test[recent10x$Uniformity<uniformity_threshold & recent10x$Uniformity>-uniformity_threshold] <- "pass"
...
```{r}
sens_spec(recent10x$uniformity_test, recent10x$symm_in_test)

sens_spec(recent10x$uniformity_test, recent10x$symm_cross_test)

sens_spec(recent10x$centre_mpc_test, recent10x$centre_cal_test)

sum(recent10x$symm_cross_test == "pass")
```

Percentage difference of symmetry to uniformity
```{r}
ratio1 <- recent6x$SymmInAT / recent6x$Uniformity
mean(ratio1)
...
ggplot(data = recent6x, aes(x = QA_Date, y = ratio)) +
  geom_point()

ggplot(data = recent6x, aes(x = QA_Date, y = ratio1)) +
  geom_point()
```
