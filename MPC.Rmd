...
```{r}
fit <- auto.arima(training_df6x$Uniformity)
pred <- forecast(fit, 5)
pred$mean
5%%5
```
...
actual <- c(training_df6x$Uniformity)
predicted <- c()
trend <- c()

for (n in 1:nrow(testing_df6x)){
  
  # add the actual value
  actual <- c(actual, testing_df6x$Uniformity[n])
...
```{r}
uniformdf <- data.frame(actual_val <- c(testing_df6x$Uniformity),
                        central <- cl,
                        lower <- lcl,
                        upper <- ucl,
                        one_step <- predicted,
                        five_step = trend[1:148],
                        Date <- c(testing_df6x$QA_Date))

```

```{r}
ggplot(uniformdf, aes(x = Date)) +
...
error <- rmse(uniformdf$actual_val, uniformdf$one_step)
abs_error <- mae(uniformdf$actual_val, uniformdf$one_step)
print(error)
print(abs_error)
...
# uniformity
uniformdf <- f(uniformdf, "pred_state", "warning")
uniformdf$pred_state[uniformdf$one_step <= -2] <- "fail"
uniformdf$pred_state[uniformdf$one_step>2] <- "fail"
uniformdf$pred_state[uniformdf$one_step<uniformdf$upper & uniformdf$one_step>uniformdf$lower] <- "pass"

uniformdf <- f(uniformdf, "actual_state", "warning")
uniformdf$actual_state[uniformdf$actual_val <= -2] <- "fail"
uniformdf$actual_state[uniformdf$actual_val>2] <- "fail"
uniformdf$actual_state[uniformdf$actual_val<uniformdf$upper & uniformdf$actual_val>uniformdf$lower] <- "pass"
